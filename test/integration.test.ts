...
describe("Integration", () => {
  describe("Accounts: Two", () => {
    ...
    test("dev/development", () => {
      const { stack, template } = generateTestApp({
        ...props,
        context: {
          account: "dev",
      ...
      expect(stack.stackName).toBe(
        "MyCoolProject-Environment-Development-TestStack",
      );
    ...
    test("dev/feature/abc-123", () => {
      ...
      expect(stack.stackName).toBe(
        "MyCoolProject-Environment-FeatureAbc123-TestStack",
      );
      expect(stack.terminationProtection).toBeFalsy();
      expect(stack.region).toBe("eu-west-1");
    ...
    test("prod/production", () => {
      ...
      expect(stack.stackName).toBe(
        "MyCoolProject-Environment-Production-TestStack",
      );
    ...
    test("dev/config", () => {
      ...
      expect(PC.getAccountConfig(stack, "baseCamp", "no camping")).toBe(
        "no camping",
      );
    ...
    test("prod/config", () => {
      ...
      expect(PC.getAccountConfig(stack, "sizing.preproduction.cpu", 512)).toBe(
        512,
      );
      expect(PC.getAccountConfig(stack, "flags[0]")).toBe("foo");
    });
    ...
    test("dev/match", () => {
      ...
      expect(
        AccountType.matchFromEnvironment(stack, props.accounts, "development"),
      ).toBe("dev");
      expect(
        AccountType.matchFromEnvironment(
          stack,
          props.accounts,
          "feature/abc-123",
        ),
      ...
      expect(
        AccountType.matchFromEnvironment(stack, props.accounts, "test"),
      ).toBe("dev");
      expect(
        AccountType.matchFromEnvironment(stack, props.accounts, "staging"),
      ).toBe("dev");
    });
    ...
    test("prod/match", () => {
      ...
      expect(
        AccountType.matchFromEnvironment(stack, props.accounts, "production"),
      ).toBe("prod");
    });
  });
});
